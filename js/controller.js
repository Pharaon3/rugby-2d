...
function countdown() {
  var interval = setInterval(function () {
    ...
    if (isGoal) {
      ...
      if (gameState[currentState]["goaltypeid"] == "23") {
        setCenterFrame("TRY", teamNames[gameState[currentState]["team"]]);
      }
      if (gameState[currentState]["goaltypeid"] == "27") {
        setCenterFrame(
          "CONVERSION",
          teamNames[gameState[currentState]["team"]]
        );
      ...
      if (
        gameState.length &&
        gameState[currentState]["type"] == "tv_timeout_start"
      ) {
        setTimer = false;
      ...
      document.getElementById("time").textContent =
        max(Math.floor(minute / 10), 0) +
        "" +
        max(0, minute % 10) +
        ":" +
        max(0, Math.floor(second / 10)) +
        "" +
        max(0, second % 10);
      if (
        gameState[currentState] &&
        gameState[currentState]["team"] == "home" &&
...
function showState() {
  ...
  if (
    ...
    if (gameState[currentState]["type"] == "Replacement") {
      ...
      document
        .getElementById("substitutionIn")
        .setAttribute("fill-opacity", 0.5);
      if (gameState[currentState]["playerin"]["name"])
        document.getElementById("substitutionInPlayer").textContent =
          gameState[currentState]["playerin"]["name"];
      if (gameState[currentState]["playerout"]["name"])
        document.getElementById("substitutionOutPlayer").textContent =
          gameState[currentState]["playerout"]["name"];
    } else document.getElementById("subsitution").style.display = "none";
    if (gameState[currentState]["type"] == "throwin") {
      showAction();
      if (gameState[currentState]["team"] == "home") {
        document.getElementById("homeKickPolygon").style.fill =
          "url(#homeKick)";
        if (y2 < hp * 0.3 && x2 > w1 * 0.6)
          document.getElementById("homeKickPolygon").style.fill =
            "url(#homeTopKick)";
        if (y2 > hp * 0.7 && x2 > w1 * 0.6)
          document.getElementById("homeKickPolygon").style.fill =
            "url(#homeBottomKick)";
        document.getElementById("homeKickPolygon").points[0].x =
          x_b + w2 + topLeft;
        document.getElementById("homeKickPolygon").points[0].y =
          y_b + topPosition;
        center_text = capitalizeWords(
          gameState[currentState]["name"].split(" ")
        ).join(" ");
        document.getElementById("homeState").textContent = center_text;
      } else {
        document.getElementById("awayKickPolygon").style.fill =
          "url(#awayKick)";
        if (y2 < hp * 0.3 && x2 < -w1 * 0.3)
          document.getElementById("awayKickPolygon").style.fill =
            "url(#awayTopKick)";
        if (y2 > hp * 0.7 && x2 < -w1 * 0.3)
          document.getElementById("awayKickPolygon").style.fill =
            "url(#awayBottomKick)";
        document.getElementById("awayKickPolygon").points[0].x =
          x_b + w2 + topLeft;
        document.getElementById("awayKickPolygon").points[0].y =
          y_b + topPosition;
        center_text = capitalizeWords(
          gameState[currentState]["name"].split(" ")
        ).join(" ");
    ...
    if (gameState[currentState]["type"] == "offside") {
      showAction();
      if (gameState[currentState]["team"] == "home") {
        document.getElementById("offsideRect").points[1].x =
          (((x2 * 50) / w1 + 50) * (718 - 232)) / 100 + 232;
        document.getElementById("offsideRect").points[2].x =
          (((x2 * 50) / w1 + 50) * (830 - 120)) / 100 + 120;
        document
          .getElementById("offsideRect")
          .setAttribute("fill-opacity", 0.5);
      } else {
        document.getElementById("offsideRect").points[0].x =
          (((x2 * 50) / w1 + 50) * (718 - 232)) / 100 + 232;
        document.getElementById("offsideRect").points[3].x =
          (((x2 * 50) / w1 + 50) * (830 - 120)) / 100 + 120;
        document
          .getElementById("offsideRect")
          .setAttribute("fill-opacity", 0.5);
    ...
    if (gameState[currentState]["type"] == "injurytimeshown") {
      ...
      document.getElementById("center_text").textContent =
        "Injury time: " + gameState[currentState]["minutes"] + "mins";
    }
...
function showAction() {
  ...
  rectWidth =
    max(rectWidth, document.getElementById("holder").getBBox().width) + 20;
  ...
  document
    .getElementById("actionBoard")
    .setAttribute("x", x_b + w2 + topLeft - rectWidth - 10);
  document
    .getElementById("actionBoard")
    .setAttribute("y", y_b + topPosition - 50 - 10);
  ...
  document
    ...
    .setAttribute("y", y_b + topPosition - 12 - 5);
  document
    .getElementById("ballState")
    .setAttribute("x", x_b + w2 + topLeft - 20);
  document
    .getElementById("ballState")
    .setAttribute("y", y_b + topPosition - 37 - 5);
  document.getElementById("stateBoardLine").setAttribute("stroke-opacity", 0.9);
  document
    .getElementById("stateBoardLine")
    .setAttribute("x1", x_b + w2 + topLeft - 15);
  document
    .getElementById("stateBoardLine")
    .setAttribute("x2", x_b + w2 + topLeft - 15);
  document
    .getElementById("stateBoardLine")
    .setAttribute("y1", y_b + topPosition - 50 - 5);
  document
    .getElementById("stateBoardLine")
    .setAttribute("y2", y_b + topPosition - 15);
  if (gameState[currentState]["team"] == "away") {
    document
      .getElementById("actionBoard")
      .setAttribute("x", x_b + w2 + topLeft + 10);
...
function displayState() {
  if (gameState[currentState]["team"] == "home") {
    ...
    document
      .getElementById("homeStateLabels")
      .setAttribute(
        "transform",
        "translate(" + statePositionX + "," + statePositionY + ")"
      );
    document
      .getElementById("homeStateBoard")
      .setAttribute(
        "width",
        max(
          document.getElementById("homeName").getBBox().width,
          document.getElementById("homeState").getBBox().width
        ) + 70
      );
    document
      .getElementById("homeStateBoard")
    ...
    if ((x2 * 50) / w1 + 50 < 25) {
      document.getElementById("awayState").textContent = "Possession";
      statePositionX = 200;
    } else if ((x2 * 50) / w1 + 50 < 50) {
      document.getElementById("awayState").textContent = "Possession";
      statePositionX = 400;
    ...
    document
      .getElementById("awayStateLabels")
      .setAttribute(
        "transform",
        "translate(" + statePositionX + "," + statePositionY + ")"
      );
    document
      .getElementById("awayStateBoard")
      .setAttribute(
        "width",
        max(
          document.getElementById("awayName").getBBox().width,
          document.getElementById("awayState").getBBox().width
        ) + 70
      );
  }
}
...
function setCenterFrame(title, content) {
  ...
  document
    ...
    .setAttribute("width", max(290, titleWidth));
  document
    .getElementById("center_rect")
...
function handleEventData(data) {
  ...
  */
  ...
  if (match) {
    ...
    document.getElementById("fade_homeTeamName").textContent =
      teamNames["home"];
    document.getElementById("fade_awayTeamName").textContent =
      teamNames["away"];
    ...
    document.getElementById("score").textContent =
      homeScore + " - " + awayScore;
    document.getElementById("fade_score").textContent =
      homeScore + " - " + awayScore;
...
function handleInfoData(data) {
  ...
  document
    .getElementById("homePlayerBase")
    .setAttribute("fill", "#" + homePlayerColor);
  document
    .getElementById("awayPlayerBase")
  ...
  document
    .getElementById("state_homePlayerBase")
    .setAttribute("fill", "#" + homePlayerColor);
  document
    .getElementById("state_awayPlayerBase")
    .setAttribute("fill", "#" + awayPlayerColor);
  if (homePlayerSleeveColor) {
    document
      .getElementById("homePlayerLeftSleeve")
      .setAttribute("fill", "#" + homePlayerSleeveColor);
    document
      .getElementById("homePlayerRightSleeve")
      .setAttribute("fill", "#" + homePlayerSleeveColor);
    ...
    document
      .getElementById("state_homePlayerLeftSleeve")
      .setAttribute("fill", "#" + homePlayerSleeveColor);
    document
      .getElementById("state_homePlayerRightSleeve")
      .setAttribute("fill", "#" + homePlayerSleeveColor);
  } else {
    document
      .getElementById("homePlayerLeftSleeve")
  ...
  if (awayPlayerSleeveColor) {
    document
      .getElementById("awayPlayerLeftSleeve")
      .setAttribute("fill", "#" + awayPlayerSleeveColor);
    document
      .getElementById("awayPlayerRightSleeve")
      .setAttribute("fill", "#" + awayPlayerSleeveColor);
    ...
    document
      .getElementById("state_awayPlayerLeftSleeve")
      .setAttribute("fill", "#" + awayPlayerSleeveColor);
    document
      .getElementById("state_awayPlayerRightSleeve")
      .setAttribute("fill", "#" + awayPlayerSleeveColor);
  } else {
    document
      .getElementById("awayPlayerLeftSleeve")
  ...
  if (homePlayerLongSleeveColor) {
    document
      .getElementById("homePlayerLeftLongSleeve")
      .setAttribute("fill", "#" + homePlayerLongSleeveColor);
    document
      .getElementById("homePlayerRightLongSleeve")
      .setAttribute("fill", "#" + homePlayerLongSleeveColor);
    ...
    document
      .getElementById("state_homePlayerLeftLongSleeve")
      .setAttribute("fill", "#" + homePlayerLongSleeveColor);
    document
      .getElementById("state_homePlayerRightLongSleeve")
      .setAttribute("fill", "#" + homePlayerLongSleeveColor);
  } else {
    document
      .getElementById("homePlayerLeftLongSleeve")
  ...
  if (awayPlayerLongSleeveColor) {
    document
      .getElementById("awayPlayerLeftLongSleeve")
      .setAttribute("fill", "#" + awayPlayerLongSleeveColor);
    document
      .getElementById("awayPlayerRightLongSleeve")
      .setAttribute("fill", "#" + awayPlayerLongSleeveColor);
    ...
    document
      .getElementById("state_awayPlayerLeftLongSleeve")
      .setAttribute("fill", "#" + awayPlayerLongSleeveColor);
    document
      .getElementById("state_awayPlayerRightLongSleeve")
      .setAttribute("fill", "#" + awayPlayerLongSleeveColor);
  } else {
    document
      .getElementById("awayPlayerLeftLongSleeve")
